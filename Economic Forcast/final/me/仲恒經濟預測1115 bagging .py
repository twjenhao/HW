...
T1=len(co2)+len(range(1,13))
fcast_h=list(range(1,13))

bt_co2 = pd.DataFrame(np.zeros((len(co2),B)),index=co2.index) #儲存 boostrapped co2 series TX10
co2_fcast=pd.DataFrame(np.zeros((T1,B)),index=pd.date_range(start=co2.index[0],periods=T1,freq="M")) #dataframe T1X10
co2.index[0]
for bb in range(B):
    ...
    co2_fcast.iloc[len(co2):,bb]=AutoETS(auto=True,n_jobs=-1,sp=12,maxiter=5000).fit_predict(bt_co2.iloc[:,bb],fh=fcast_h)
    #orginal co2 data 
    co2_fcast.iloc[:len(co2),bb]=co2
co2_fcast["1980":].plot(legend=False,figsize=(10,8))
plt.show()
